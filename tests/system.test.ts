...
class FailingBroker implements ExchangeBroker {
  async placeOrder(): Promise<ExchangeOrder> {
    throw new Error('placeOrder should not be called');
  }
  async cancelOrder(): Promise<ExchangeOrder | null> {
    return null;
  }
...
class TrackingBroker implements ExchangeBroker {
  public placed: ExchangeOrder[] = [];
  public positions: ExchangePosition[] = [];
  async placeOrder(order: Omit<ExchangeOrder, 'status' | 'timestamp'>): Promise<ExchangeOrder> {
    const created: ExchangeOrder = { ...order, status: 'FILLED', timestamp: Date.now() };
